...
<div class="max-w-7xl mx-auto py-2 px-2">
    <nav class="flex mb-4" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2 text-sm text-slate-400 font-medium">
            ...
            <li class="text-slate-900">Edit Mata Kuliah</li>
        </ol>
    </nav>

    <div class="bg-white shadow-xl ring-1 ring-slate-200 rounded-2xl overflow-hidden" x-data="{ isOrigin: {{ $subject->university_id ? 'true' : 'false' }} }">
        <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <div>
                <h3 class="text-xl font-bold text-slate-900 tracking-tight">Edit Mata Kuliah: {{ $subject->code }}</h3>
                <p class="mt-1 text-sm text-slate-500">Perbarui informasi mata kuliah di repositori.</p>
            </div>
        ...
        <div class="px-8 py-10">
            <form method="POST" action="{{ route('kaprodi.subjects.update', $subject) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="col-span-1">
                        ...
                        <input id="code" name="code" type="text" required value="{{ old('code', $subject->code) }}" class="appearance-none rounded-lg relative block w-full px-4 py-3 border border-slate-300 placeholder-slate-400 text-slate-900 shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="mis. INF-101">
                    ...
                    <div class="col-span-1">
                        ...
                        <input id="name" name="name" type="text" required value="{{ old('name', $subject->name) }}" class="appearance-none rounded-lg relative block w-full px-4 py-3 border border-slate-300 placeholder-slate-400 text-slate-900 shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="mis. Sistem Basis Data">
                        @error('name') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div class="col-span-full">
                        <label for="description" class="block text-sm font-medium text-slate-700 mb-1">Deskripsi (Opsional)</label>
                        <textarea id="description" name="description" rows="3" class="appearance-none rounded-lg relative block w-full px-4 py-3 border border-slate-300 placeholder-slate-400 text-slate-900 shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Tinjauan singkat tentang kurikulum mata kuliah...">{{ old('description', $subject->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <input id="sks" name="sks" type="number" required value="{{ old('sks', $subject->sks) }}" min="1" max="24" class="appearance-none rounded-lg relative block w-full px-4 py-3 border border-slate-300 placeholder-slate-400 text-slate-900 shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <input id="semester" name="semester" type="number" value="{{ old('semester', $subject->semester) }}" min="1" max="10" class="appearance-none rounded-lg relative block w-full px-4 py-3 border border-slate-300 placeholder-slate-400 text-slate-900 shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    <div class="col-span-full" x-show="isOrigin" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <select id="university_id" name="university_id" :required="isOrigin" class="appearance-none rounded-lg relative block w-full px-4 py-3 border border-amber-300 bg-amber-50/20 text-slate-900 shadow-sm focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm">
                            ...
                            @foreach($universities as $uni)
                                <option value="{{ $uni->id }}" {{ old('university_id', $subject->university_id) == $uni->id ? 'selected' : '' }}>{{ $uni->name }}</option>
                            @endforeach
                        </select>
