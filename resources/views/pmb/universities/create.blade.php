...
@section('title', 'Daftarkan Universitas')
...
<div class="max-w-2xl mx-auto py-2 px-2">
    <nav class="flex mb-4" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2 text-sm text-slate-400 font-medium">
            ...
            <li class="text-slate-900">Daftarkan Universitas</li>
        </ol>
    </nav>
    ...
    <div class="bg-white shadow-xl ring-1 ring-slate-200 rounded-3xl overflow-hidden">
        <div class="px-8 py-8 border-b border-slate-100 bg-slate-50/50">
            <h3 class="text-2xl font-bold text-slate-900 tracking-tight text-center">Daftarkan Universitas Baru</h3>
            <p class="mt-1 text-sm text-slate-500 text-center">Tambahkan universitas resmi ke repositori sistem.</p>
        </div>

        <div class="px-8 py-10">
            <form method="POST" action="{{ route('pmb.universities.store') }}" class="space-y-6">
                @csrf

                <div class="space-y-4">
                    <div>
                        <label for="name" class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Nama Universitas</label>
                        <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" placeholder="mis. Universitas Indonesia" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" placeholder="mis. UI">
